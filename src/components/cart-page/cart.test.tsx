...
describe("Cart component", () => {
  ...
  it("Clicking minus reduces item quantity", async () => {
    ...
    const minusButtonOfFirstItem = within(listItems[0]!).getByRole("button", {
      name: "-",
    });
  ...
  it("Clicking plus increases item quantity", async () => {
    const listItems = screen.getAllByRole("listitem");
    if (!listItems) throw new Error("No list items found");
    const plusButtonOfFirstItem = within(listItems[0]!).getByRole("button", {
      name: "+",
    });
  ...
  it("Correctly calculates subtotal dynamically per Item", async () => {
    const listItems = screen.getAllByRole("listitem");
    if (!listItems) throw new Error("No list items found");
    const priceLine = within(listItems[0]!).getByText(
      /€ each \(Subtotal:/i,
    ).textContent;
    const plusButtonOfFirstItem = within(listItems[0]!).getByRole("button", {
      name: "+",
    ...
    if (!quantityP) {
      throw new Error("Quantity p element not found");
    }

    const match = priceLine.match(
      /([\d.]+)\s*€ each \(Subtotal:\s*([\d.]+)\s*€\)/,
    );
    ...
    expect(Number(quantityP.textContent) * unitPrice).toBe(subtotal);

    const user = userEvent.setup();
    await user.click(plusButtonOfFirstItem);

    const newPriceLine = within(listItems[0]!).getByText(
      /€ each \(Subtotal:/i,
    ).textContent;
    const newMatch = newPriceLine.match(
      /([\d.]+)\s*€ each \(Subtotal:\s*([\d.]+)\s*€\)/,
    );
    ...
    expect(Number(quantityP.textContent) * newUnitPrice).toBe(newSubtotal);
  });
  it("Order summary correctly displays subtotal of all items", async () => {
    ...
    if (!subtotalOfOrderPriceP)
      throw new Error("Subtotal paragraph element not found");
    const subtotalOfOrder = Number(
      subtotalOfOrderPriceP.textContent.replace(/\s*€/, ""),
    );
    ...
    for (let item of listItems) {
      const priceLine =
        within(item).getByText(/€ each \(Subtotal:/i).textContent;
      const match = priceLine.match(
        /([\d.]+)\s*€ each \(Subtotal:\s*([\d.]+)\s*€\)/,
      );
      if (!match) throw new Error("Regex didn't match");
      const subtotal = Number(match[2]);
    ...
    const user = userEvent.setup();
    await user.click(plusButtonOfFirstItem);
    const newSubtotalOfOrder = Number(
      subtotalOfOrderPriceP.textContent.replace(/\s*€/, ""),
    );
  ...
  it("Order summary displays 4.99 € shipping cost if the order is under 50,00 €", () => {
    ...
    if (!shippingPriceInTextP)
      throw new Error("Shipping costs paragraph element not found");
    const shippingOfOrder = Number(
      shippingPriceInTextP.textContent.replace(/\s*€/, ""),
    );
    const listItems = screen.getAllByRole("listitem");
    let calculatedTotal = 0;
    for (let item of listItems) {
  ...
  it("Order summary displays 19% tax correctly", () => {
    ...
    if (!taxPriceInTextP)
      throw new Error("Tax costs paragraph element not found");
    const taxOfOrder = Number(taxPriceInTextP.textContent.replace(/\s*€/, ""));

    expect(taxOfOrder).toBeCloseTo(
      ((subtotalOfOrder + shippingOfOrder) / 100) * 19,
    );
  });
  it("Order summary display total price correctly", () => {
    ...
    if (!totalPriceInTextP)
      throw new Error("Total costs paragraph element not found");
    const totalOfOrder = Number(
      totalPriceInTextP.textContent.replace(/\s*€/, ""),
    );

    expect(totalOfOrder).toBeCloseTo(
      subtotalOfOrder + shippingOfOrder + taxOfOrder,
    );
  });
});
